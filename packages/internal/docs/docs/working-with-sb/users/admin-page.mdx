...
Access policies are used in the Market Frontier to restrict access to queries and mutations.
...
:::info
Access policies in the Market Frontier are implemented using the [Django REST - Access Policy](https://rsinger86.github.io/drf-access-policy/) library.
...
:::caution
By default, the Market Frontier uses the `IsAuthenticated` permission class from Django REST Framework to restrict access to API endpoints.
