...
Market Frontier uses [nx](https://nx.dev/) to manage the monorepo, `pnpm` workspaces and TypeScript paths to link to
proper directories when importing any dependencies.

In truth, it doesn't matter where you put the library, but Market Frontier follows a pattern of putting all libraries
...
A new webapp library needs a few different files, so Market Frontier comes with a generator to make the job easier.
