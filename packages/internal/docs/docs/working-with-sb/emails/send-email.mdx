...
In the Market Frontier, you can send emails from the backend by creating a new class that subclasses the `Email` class.
...
In the Market Frontier, email data is serialized using the Django REST Framework serializers.
...
Here's an example of an email serializer used in the Market Frontier:
...
In the Market Frontier, the `Email` class is a subclass of the `common.tasks.Task` class, which emits the email event to be handled by async workers.
