---
description: Steps required to deploy Market Frontier on AWS
---
...
```shell
pnpm saas aws set-var MF_HOSTED_ZONE_ID XYZ
pnpm saas aws set-var MF_HOSTED_ZONE_NAME example.com
pnpm saas aws set-var MF_DOMAIN_ADMIN_PANEL admin.qa.example.com
pnpm saas aws set-var MF_DOMAIN_FLOWER flower.qa.example.com
pnpm saas aws set-var MF_DOMAIN_API api.qa.example.com
pnpm saas aws set-var MF_DOMAIN_CDN cdn.qa.example.com
pnpm saas aws set-var MF_DOMAIN_DOCS docs.qa.example.com
pnpm saas aws set-var MF_DOMAIN_WEB_APP app.qa.example.com
```

:::info
`MF_HOSTED_ZONE_ID` and `MF_HOSTED_ZONE_NAME` can be skipped if you are using externally managed DNS. In that case,
certificates for CloudFront distribution and Load Balancer should be already generated, and their ARNs provided in
`MF_CLOUDFRONT_CERTIFICATE_ARN` and `MF_LOAD_BALANCER_CERTIFICATE_ARN` parameters. As the last step,
...
```
pnpm saas aws set-var MF_CERTIFICATE_DOMAIN example.com
```
...
```shell
pnpm saas aws set-var MF_CI_MODE <parallel/simple>
```
...
```shell
pnpm saas aws set-var MF_BASIC_AUTH username:password
```
...
```shell
pnpm saas aws set-var MF_TOOLS_ENABLED <true/false>
pnpm saas aws set-var MF_TOOLS_BASIC_AUTH username:password
pnpm saas aws set-var MF_TOOLS_HOSTED_ZONE_ID XYZ
pnpm saas aws set-var MF_TOOLS_HOSTED_ZONE_NAME example.com
pnpm saas aws set-var MF_TOOLS_DOMAIN_VERSION_MATRIX status.example.com
```

:::info

If you set `MF_TOOLS_ENABLED` to `true` make sure to deploy **version matrix** component before first CI pipeline run.
