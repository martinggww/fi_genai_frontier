...
Market Frontier generates a user for you that has minimal permissions required for uploading code artifact to the S3
...
In order to upload the code artifact to the S3 bucket, the CI pipeline needs to authenticate with AWS. Market Frontier
...
5. Create following secrets under `Environment secrets`:
   ...
   - `MF_CI_ARTIFACTS_BUCKET` – [artifacts bucket name](#get-artifacts-bucket-name)

6. Create following secrets under `Environment variables`:

   - `MF_DEPLOY_STAGE` – set to the name of the environment
...
7. Create following variables for the environment:
   ...
   - `MF_DEPLOY_ENVIRONMENT_NAME` - set to the name of the full project environment name (`saas-qa` for example)